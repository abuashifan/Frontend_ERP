...
async function load() {
  ...
  if (!props.customerId) {
    ElMessage.error('customerId tidak ditemukan')
    return
  }

  loading.value = true
  try {
    const customer: Customer = await getCustomer(props.customerId)
    model.value = {
      code: customer.code,
      name: customer.name,
    ...
    ElMessage.error(String(maybe?.response?.data?.message ?? maybe?.message ?? 'Gagal memuat customer'))
  } finally {
    loading.value = false
  }
}
...
async function save() {
  ...
  try {
    if (isEdit.value && props.customerId) {
      await updateCustomer(props.customerId, model.value)
  ...
  <div class="p-4">
    <div class="flex items-center justify-between gap-3 mb-3">
      <div class="flex items-center gap-3">
        <div class="text-lg font-semibold">{{ isEdit ? 'Edit Customer' : 'New Customer' }}</div>
